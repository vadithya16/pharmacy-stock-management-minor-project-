...
$update= "UPDATE `stock` SET `s_status`='Expired' WHERE CURRENT_DATE>=`s_expiry`";
$result = mysqli_query($mysqli,$update) or die ( mysqli_error());
?>
<!DOCTYPE html>
<html>
<head>
...
<body>
   ...
   <h1 class="clrblk">Expired Stock Search</h1>
      ...
      <table>
         ...
         if(isset($_GET['searchmed']))
         {
            ...
            $query="SELECT * FROM stock WHERE concat(s_medname,s_qty,s_price,s_batch,s_expiry,s_category) LIKE '%$filtervalues%' && `s_status`='Expired'";
            $result1 = mysqli_query($mysqli, $query);
               if (mysqli_num_rows($result1) > 0) 
               {
                  foreach ($result1 as $row) 
                  {
                     ...
                     <a href='deleteexpiry.php?del=$row[s_id]' onClick=\"return confirm('Are you SURE DELETE this RECORD?')\" class='tda tda1'>Delete</a></td></tr>";
                     $count++;  
                  }
               } 
               else 
         ...
         {
            $sql1 = "SELECT * FROM stock WHERE `s_status`='Expired'";
